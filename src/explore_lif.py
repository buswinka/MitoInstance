...
# Modified by Chris Buswinka to somehow work with 4 Channel MEEI Lif files
...
channelTag = ["Blue", "Green", "Yellow", "Red"]
...
class Header:
    ...
    def parse(self, xmlHeaderFile, quick=False):
        """
...
class Reader(Header):
    ...
    def __init__(self, lifFile, quick=True):
        ...
        # self.offsets = [int(m.getAttribute("Size")) for m in self.xmlHeader.getElementsByTagName("Memory")]
        # self.offsets = [i for i in self.offsets if i != 0]
...
class Serie(SerieHeader):
    ...
    def getFrame(self, T=0, channel=0, dtype=np.uint16):
        ...
        for z in range(self.getFrameShape()[0]):
            channel_offset = self.getChannelOffset(channel)

            # For whatever reason, the blue channel offset is off by one. WTF
            if channel == 0:
                channel_offset += 1
